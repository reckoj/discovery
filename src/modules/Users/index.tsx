...
const Users = ({}: any) => {
  ...
  const onHandle = (item: any, id: string) => {
    if (item.title.toLowerCase() === 'view') {
      ...
      if (confirm) onRemoveUser(id);
    } else if (item.title.toLowerCase() === 'make employee') {
      let confirm = window.confirm(
        'Are you sure you want to make this user employee?'
      );
      if (confirm) onUpdateUser(id, { user_type: 'org_admin' });
    }
  };
  ...
  const onGettingUsers = async (page: number, search = '') => {
    try {
      ...
      let { status, data } = await getAlUsers({ page, search }, authToken);
      if (status === 200) {
  ...
  const submitUser = async () => {
    try {
      ...
      if (error.length)
        return error.forEach((err) => toast.warning(err, toastUtil));
      setLoading(true);
      let _payload = {
        ...payload,
  ...
  const getSelected = async (item: any) => {
    try {
      ...
      let { status, data } = await getAlUsers(
              ...
              <div className="relative p-4 rounded-md shadow-cs-1 flex flex-col items-center bg-white outline-none max-h-[90%] w-[400px] overflox-x-hidden overflow-y-auto focus:outline-none screen800:px-3">
                ...
                <form className="flex items-center flex-col rounded-lg shadow-1 bg-white p-4 w-[350px] min-h-[490px]">
                  ...
                  <div className="mb-8 relative w-full">
                    ...
                    <Selector
                      options={[{ label: 'org_admin', value: 'Admin' }]}
