...
const Organizations = ({}: any) => {
  ...
  const [payload, setPayload]: any = useState({
    ...
    island: '',
    description: '',
    user_type: AppRoles.ORGANIZATION,
    established: '',
    password: '',
  });
  ...
  const onGetting = async (search = '') => {
    try {
      setLoading(true);

      let { status, data } = await getAlOrganizations({ search }, authToken);
      if (status === 200) {
  ...
  const update = async () => {
    try {
      setLoading(true);
      let { status, data } = await updateOrganization(
        payload,
        payload?._id,
        authToken
      );
        ...
        >
          <div className="relative p-8 rounded-md shadow-cs-1 flex flex-col items-center bg-white outline-none focus:outline-none screen800:px-3 max-h-[600px] overflow-x-hidden overflow-y-auto w-[400px]">
            ...
            <h3 className="text-primary font-medium text-xl">
              {payload?._id ? 'Edit' : 'Add'} Organization Details
            </h3>
            <form className="flex items-center flex-col rounded-lg shadow-1 bg-white p-4 w-[350px] min-h-[490px]">
              ...
              <div className="mb-8 relative w-full">
                ...
                <input
                  id="est"
                  value={payload?.established}
                  onChange={(e) =>
                    setPayload((prev: any) => {
                      return { ...prev, established: e.target.value };
                    })
                  }
                  placeholder=""
                  className=" h-[42px] shadow-sm appearance-none border border-neutral-300 rounded-md w-full py-2 px-3 text-gray-500 leading-tight focus:outline-none focus:shadow-outline"
