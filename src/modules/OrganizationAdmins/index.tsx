...
import { getAlUsers, removeUser } from '../../apis/users.api';
...
const OrganizationAdmins = ({}: any) => {
  ...
  const onHandle = (item: any, _user: any) => {
    ...
    if (item.title.toLowerCase() === 'view') {
      navigate(`/employee/${_user?._id}`);
    } else if (item.title.toLowerCase() === 'edit') {
      let user = {
        ..._user,
        island: { label: _user.island, value: _user.island },
      };
      ...
      if (confirm) onRemoveUser(_user._id);
    }
    setLoading(false);
  };

  const onGettingUsers = async (page: number = 1, search = '') => {
    try {
      let options: any = getRoleBasedActions();
      let { status, data } = await getOrganizationAdmins(
  ...
  const updateUser = async () => {
    try {
      ...
      let _payload = {
        ...payload,
        user_type: payload?.user_type?.label,
        island: payload?.island?.label,
      };

      let { status } = await updateOrganizationUser(
        _payload,
        payload?._id,
        authToken
      );
  ...
  const getSelected = async (item: any) => {
    ...
    try {
      ...
      let { status, data } = await getOrganizationAdmins(
        { search: item.value || null, page: Number(pagination?.currentPage) },
        authToken
      );
      if (status === 200) {
        let dtrows = data?.data?.results.map((item: any) => {
            ...
            >
              <div className="relative p-4 rounded-md shadow-cs-1 flex flex-col items-center bg-white outline-none max-h-[90%] w-[400px] overflox-x-hidden overflow-y-auto focus:outline-none screen800:px-3">
                ...
                <h3 className="text-primary font-medium text-xl">
                  {payload?._id ? 'Edit' : 'Add'} Employee Details
                </h3>
                <form className="flex items-center flex-col rounded-lg shadow-1 bg-white p-4 w-[350px] min-h-[490px]">
                  ...
                  <div className="mb-8 relative w-full">
                    ...
                    <Selector
                      options={[{ label: 'org_admin', value: 'Employee' }]}
                      // defaultValue={payload?.user_type}
                      onChange={(item: any) => {
                        setPayload((prev: any) => {
                          return { ...prev, user_type: item };
                        });
                      }}
                    />
                  </div>
